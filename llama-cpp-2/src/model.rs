...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RopeType {
    /// Standard RoPE as introduced in the original LLaMA — sequential
    /// interleaving of the rotation pairs across the head dimension.
    Norm,
    /// GPT-NeoX-style RoPE — half-and-half pair layout used by NeoX,
    /// Pythia, and many derived architectures.
    NeoX,
    /// Multimodal RoPE — multi-axis rotation (time / height / width)
    /// used by Qwen2-VL, Qwen2.5-VL, and related vision-language models.
    MRope,
    /// Vision-tower RoPE used by the ViT-style image encoder branch of
    /// multimodal architectures (distinct from `MRope` on the LM side).
    Vision,
}
